...
def _init_debug_logger():
    ...
    logger.info("=" * 80)
    logger.info("信号调试日志已初始化 - 自适应动量策略")
...
class AdaptiveMomentumStrategy(StrategyBase):
    ...
    def __init__(
        ...
        atr_slippage_multiplier: float = 1.0,
        ...
        **kwargs,
        ...
        if atr_entry_multiplier <= 0:
            raise ValueError("ATR 入场倍数必须大于 0")
        
        if atr_exit_multiplier <= 0:
            raise ValueError("ATR 追踪止损启动倍数必须大于 0")
        
        if risk_per_trade_percent <= 0 or risk_per_trade_percent > 1:
            raise ValueError("单笔风险比例必须在 0-1 之间")
        
        if trailing_stop_atr_multiplier <= 0:
            raise ValueError("追踪止损 ATR 倍数必须大于 0")
        ...
        self.atr_exit_multiplier = atr_exit_multiplier
        ...
        self.logger.info(
            f"自适应动量策略初始化: initial_data_days={initial_data_days}天, "
        ...
        self.atr_series: List[Optional[float]] = []
        ...
        self.logger.info(
            f"自适应动量策略初始化: "
            f"短期周期={short_period}, 长期周期={long_period}, "
            f"合约={contract}, K线周期={kline_duration}秒, "
            f"均线类型={'EMA' if use_ema else 'SMA'}, "
            f"RSI周期={rsi_period}, RSI阈值={rsi_threshold}, "
            f"ATR周期={atr_period}, "
            f"入场过滤=ATR×{atr_entry_multiplier}, "
            f"追踪止损启动=盈利>ATR×{atr_exit_multiplier}, "
            f"追踪止损距离=ATR×{trailing_stop_atr_multiplier}, "
            f"单笔风险={risk_per_trade_percent*100}%, "
            f"合约乘数={contract_multiplier}"
        )
    ...
    def _init_dataframe(self):
        ...
        if self._price_list and self._high_list and self._low_list:
            min_len = min(len(self._price_list), len(self._high_list), len(self._low_list))
            self._price_df = pd.DataFrame({
                'close': self._price_list[:min_len],
                'high': self._high_list[:min_len],
                'low': self._low_list[:min_len],
                'open': self._open_list[:min_len] if len(self._open_list) >= min_len else self._price_list[:min_len],
            })
    ...
    def _calculate_all_indicators(self):
        ...
        high_prices = self._price_df['high'].values
        low_prices = self._price_df['low'].values
        
        if TALIB_AVAILABLE:
            np_close = np.array(close_prices, dtype=float)
            np_high = np.array(high_prices, dtype=float)
            np_low = np.array(low_prices, dtype=float)
            ...
            if len(self._price_df) >= self.atr_period:
                self._price_df['atr'] = talib.ATR(np_high, np_low, np_close, timeperiod=self.atr_period)
            else:
                self._price_df['atr'] = None
            ...
            if len(self._price_df) >= self.atr_period:
                self._price_df['atr'] = self._calculate_atr_pandas(self._price_df, self.atr_period)
            else:
                self._price_df['atr'] = None
        
        self.short_ma_series = self._price_df['short_ma'].tolist()
        self.long_ma_series = self._price_df['long_ma'].tolist()
        self.atr_series = self._price_df.get('atr', pd.Series()).tolist()
    ...
    @staticmethod
    def _calculate_atr_pandas(df: pd.DataFrame, period: int) -> pd.Series:
        high = df['high']
        low = df['low']
        close = df['close']
    ...
    def _calculate_tr_manual(self, idx: int) -> float:
        if idx <= 0:
            return self._high_list[idx] - self._low_list[idx]
        
        current_high = self._high_list[idx]
        current_low = self._low_list[idx]
        prev_close = self._price_list[idx - 1]
            ...
            if self._entry_price is not None:
                ...
                if self._trailing_stop_active:
                    ...
                    if self._trailing_stop_price is None or new_stop > self._trailing_stop_price:
                        ...
                        self.logger.info(
                            f"[追踪止损] 止损位上移: 旧={self._trailing_stop_price:.2f}, 新={new_stop:.2f}, "
            ...
            if self._entry_price is not None:
                ...
                if self._trailing_stop_active:
                    ...
                    if self._trailing_stop_price is None or new_stop < self._trailing_stop_price:
                        self._trailing_stop_price = new_stop
                        self.logger.info(
                            f"[追踪止损] 止损位下移: 旧={self._trailing_stop_price:.2f}, 新={new_stop:.2f}, "
                            f"最低价={self._lowest_price_since_entry:.2f}"
                        )
    
    def _check_trailing_stop(self, current_price: float) -> bool:
        if not self._trailing_stop_active or self._trailing_stop_price is None:
            return False
        ...
        if self._position > 0:
            if current_price <= self._trailing_stop_price:
                triggered = True
                self.logger.info(
                    f"[追踪止损] 触发: 当前价={current_price:.2f} <= 止损位={self._trailing_stop_price:.2f}"
                )
        ...
        if triggered:
            self._trailing_stop_triggered += 1
            self._trailing_stop_active = False
            self._trailing_stop_price = None
        
        return triggered
    
    def _log_indicator_trajectory(self, current_price: float):
        ...
        logger.info("")
        logger.info("=" * 80)
        logger.info(f"[指标轨迹追踪] cycle={self._cycle_count} - 最近{n}根K线")
        logger.info("-" * 80)
        header = f"{'周期':<8} {'收盘价':<12} {'MA' + str(self.short_period):<15} {'MA' + str(self.long_period):<15} {'RSI':<12} {'ATR':<12} {'状态':<10}"
        logger.info(header)
        logger.info("-" * 80)
        ...
        for i in range(-n, 0):
            ...
            rsi_val = self._price_df['rsi'].iloc[idx] if self._price_df is not None and idx < len(self._price_df) else None
            atr_val = self.atr_series[idx] if idx < len(self.atr_series) else None
            ...
            rsi_str = f"{rsi_val:.2f}" if rsi_val is not None and pd.notna(rsi_val) else "N/A"
            atr_str = f"{atr_val:.2f}" if atr_val is not None and pd.notna(atr_val) else "N/A"
            ...
            logger.info(f"{cycle_num:<8} {close_val:<12.2f} {short_ma_str:<15} {long_ma_str:<15} {rsi_str:<12} {atr_str:<12} {status:<10}{marker}")
    ...
    def update_prices(self, close_price: float, high_price: float = None, low_price: float = None, open_price: float = None):
        ...
        actual_open = open_price if open_price is not None else close_price
        
        actual_high = max(actual_high, close_price)
        actual_low = min(actual_low, close_price)
        ...
        self._high_list.append(actual_high)
        self._low_list.append(actual_low)
        self._open_list.append(actual_open)
        ...
        if PANDAS_AVAILABLE:
            ...
            if len(self._price_df) >= self.short_period:
                ...
                self.atr = float(last_row['atr']) if pd.notna(last_row.get('atr')) else None
        else:
            self._fallback_calculate_ma()
            self._fallback_calculate_atr()
        
        if self.debug_logging and len(self._price_list) >= self.long_period:
            self._log_indicator_trajectory(close_price)
        ...
        if self.debug_logging:
            debug_data = {
                ...
                'trailing_stop': self._trailing_stop_price,
            }
        ...
        if self._position != 0:
            self._update_trailing_stop(close_price)
            if self._check_trailing_stop(close_price):
    ...
    def _fallback_calculate_atr(self):
        if len(self._price_list) < self.atr_period + 1:
            self.atr = None
            self.atr_series.append(None)
            return
        
        tr_list = []
        for i in range(len(self._price_list) - self.atr_period, len(self._price_list)):
            tr = self._calculate_tr_manual(i)
            tr_list.append(tr)
        
        self.atr = sum(tr_list) / len(tr_list)
        self.atr_series.append(self.atr)
        ...
        try:
            ...
            if self._position == 0:
                if self._cycle_count % 2 == 1:
                    ...
                    self._highest_price_since_entry = close_price
                    self._lowest_price_since_entry = close_price
    ...
    def update_from_kline(self, kline_data: Dict[str, Any]):
        ...
        try:
            ...
            self.update_prices(close_price, high_price, low_price, open_price)
        except (ValueError, TypeError) as e:
            self.logger.warning(f"无效的价格数据: {e}")
    ...
    def _detect_signal(self, close_price: float = 0):
        ...
        if price_change < atr_filter_threshold:
            self._atr_filtered_signals += 1
            self.signal = SignalType.HOLD
            self.logger.info(
                f"[ATR_FILTER] cycle={self._cycle_count} 原信号={ma_signal.value}, "
                f"价格波动={price_change:.2f} < ATR×{self.atr_entry_multiplier}={atr_filter_threshold:.2f}，信号被过滤（横盘震荡）"
            )
            if self.debug_logging:
                log_signal_debug(self.contract, self._cycle_count, {
                    'signal': 'ATR_FILTERED',
    ...
    def _check_take_profit_stop_loss(self, current_price: float):
        ...
        if self._position > 0:
            if self.atr is not None and self.atr > 0:
                tp_price = self._entry_price + self.atr_exit_multiplier * self.atr
                if current_price >= tp_price:
                    tp_triggered = True
                    self.logger.info(
                        f"止盈触发: 多单入场价={self._entry_price:.2f}, 当前价={current_price:.2f}, "
                        f"止盈价={tp_price:.2f} (ATR×{self.atr_exit_multiplier})"
                    )
                
                if not tp_triggered:
                    sl_price = self._entry_price - self.atr_entry_multiplier * self.atr
                    if current_price <= sl_price:
                        sl_triggered = True
                        self.logger.info(
                            f"止损触发: 多单入场价={self._entry_price:.2f}, 当前价={current_price:.2f}, "
                            f"止损价={sl_price:.2f} (ATR×{self.atr_entry_multiplier})"
                        )
            ...
            if self.atr is not None and self.atr > 0:
                ...
                if current_price <= tp_price:
                    tp_triggered = True
                    self.logger.info(
                        f"止盈触发: 空单入场价={self._entry_price:.2f}, 当前价={current_price:.2f}, "
                        f"止盈价={tp_price:.2f} (ATR×{self.atr_exit_multiplier})"
                    )
                ...
                if not tp_triggered:
                    ...
                    if current_price >= sl_price:
                        sl_triggered = True
                        self.logger.info(
                            f"止损触发: 空单入场价={self._entry_price:.2f}, 当前价={current_price:.2f}, "
                            f"止损价={sl_price:.2f} (ATR×{self.atr_entry_multiplier})"
                        )
    ...
    def _execute_trade(self, signal: SignalType, close_price: float = 0):
        ...
        try:
            ...
            position_size = 1
            if self.atr is not None and self.atr > 0:
                calculated_size = self._calculate_position_size(self.atr, current_price)
                position_size = max(1, int(round(calculated_size)))
                self._position_size = position_size
                self.logger.info(
                    f"[动态仓位] 计算下单手数: 总资金={self._total_capital:.2f}, "
                    f"风险比例={self.risk_per_trade_percent*100}%, "
                    f"ATR={self.atr:.2f}, 合约乘数={self.contract_multiplier}, "
                    f"下单手数={position_size}"
                )
            ...
            if signal == SignalType.BUY:
                ...
                if self._position == 0:
                    self._place_order(quote, direction="BUY", offset="OPEN", volume=position_size, limit_price=buy_price)
                    self._position = position_size
                ...
                if self._position == 0:
                    self._place_order(quote, direction="SELL", offset="OPEN", volume=position_size, limit_price=sell_price)
                    self._position = -position_size
    ...
    def get_ma_values(self) -> Dict[str, Optional[float]]:
        ...
        if self.atr is not None:
            result['atr'] = self.atr
            result['atr_period'] = self.atr_period
            result['atr_entry_multiplier'] = self.atr_entry_multiplier
            result['atr_exit_multiplier'] = self.atr_exit_multiplier
    ...
    def get_atr_value(self) -> Optional[float]:
        return self.atr
    ...
    def get_filter_stats(self) -> Dict[str, Any]:
        return {
            ...
            'atr_filtered_signals': self._atr_filtered_signals,
            'rsi_filtered_signals': self._rsi_filtered_signals,
        }
    
    def get_tp_sl_stats(self) -> Dict[str, Any]:
        return {
            'take_profit_ratio': self.atr_exit_multiplier,
            'stop_loss_ratio': self.atr_entry_multiplier,
    ...
    def on_bar(self, bar_data: Dict[str, Any]):
        ...
        try:
            ...
            if self.is_ready():
                ...
                rsi_info = f", RSI={self.rsi:.2f}" if self.rsi is not None else ""
                atr_info = f", ATR={self.atr:.2f}" if self.atr is not None else ""
                
                self.logger.debug(
                    f"K线更新 - MA{self.short_period}: {ma_values[f'ma_{self.short_period}']:.2f}, "
                    f"MA{self.long_period}: {ma_values[f'ma_{self.long_period}']:.2f}{rsi_info}{atr_info}, "
    ...
    def _warmup_klines(self):
        ...
        self.logger.info(
            f"开始预热 K 线 (自适应动量策略): initial_data_days={self.initial_data_days}天, "
            f"需要预热K线数={self._required_warmup_klines}根, "
        ...
        rsi_status = f", RSI={self.rsi:.2f}" if self.rsi is not None else ""
        atr_status = f", ATR={self.atr:.2f}" if self.atr is not None else ""
        self.logger.info(
            f"冷启动预热完成: 已收集 {len(self._price_list)} 条价格记录, "
            f"短期均线={self.short_ma}, 长期均线={self.long_ma}{rsi_status}{atr_status}"
        )
    ...
    def run(self):
        ...
        self.logger.info(f"ATR 入场过滤倍数: {self.atr_entry_multiplier}")
        self.logger.info(f"ATR 追踪止损启动倍数: {self.atr_exit_multiplier}")
        self.logger.info(f"追踪止损 ATR 倍数: {self.trailing_stop_atr_multiplier}")
    ...
    def save_state(self) -> Dict[str, Any]:
        state = {
            ...
            '_high_list': list(self._high_list) if self._high_list else [],
            '_low_list': list(self._low_list) if self._low_list else [],
            ...
            'atr_series': list(self.atr_series) if self.atr_series else [],
            '_atr_filtered_signals': self._atr_filtered_signals,
            '_rsi_filtered_signals': self._rsi_filtered_signals,
            '_tp_triggered': self._tp_triggered,
            '_sl_triggered': self._sl_triggered,
            '_trailing_stop_triggered': self._trailing_stop_triggered,
            '_position': self._position,
            '_position_size': self._position_size,
            '_entry_price': self._entry_price,
            '_trailing_stop_active': self._trailing_stop_active,
            '_trailing_stop_price': self._trailing_stop_price,
            '_highest_price_since_entry': self._highest_price_since_entry,
            '_lowest_price_since_entry': self._lowest_price_since_entry,
            '_total_capital': self._total_capital,
        }
        
        self.logger.info(
            f"保存策略状态: 已收集 {len(self._price_list)} 条价格记录, "
            f"ATR过滤信号数={self._atr_filtered_signals}, RSI过滤信号数={self._rsi_filtered_signals}, "
            f"止盈触发={self._tp_triggered}, 止损触发={self._sl_triggered}, "
            f"追踪止损触发={self._trailing_stop_triggered}"
        )
        return state
    ...
    def load_state(self, state: Dict[str, Any]) -> None:
        ...
        if 'atr_period' in state:
            if state['atr_period'] != self.atr_period:
                self.logger.warning(f"保存的 ATR 周期 ({state['atr_period']}) 与当前配置 ({self.atr_period}) 不一致")
        
        if 'atr_entry_multiplier' in state:
            if state['atr_entry_multiplier'] != self.atr_entry_multiplier:
                self.logger.warning(f"保存的 ATR 入场倍数 ({state['atr_entry_multiplier']}) 与当前配置 ({self.atr_entry_multiplier}) 不一致")
        
        if 'atr_exit_multiplier' in state:
            if state['atr_exit_multiplier'] != self.atr_exit_multiplier:
                self.logger.warning(f"保存的 ATR 追踪止损启动倍数 ({state['atr_exit_multiplier']}) 与当前配置 ({self.atr_exit_multiplier}) 不一致")
        ...
        if '_high_list' in state:
            highs = state['_high_list']
            if highs:
                self._high_list = [float(h) for h in highs]
        
        if '_low_list' in state:
            lows = state['_low_list']
            if lows:
                self._low_list = [float(l) for l in lows]
        ...
        if 'atr_series' in state:
            series = state['atr_series']
            if series:
                self.atr_series = [float(a) if a is not None else None for a in series]
        ...
        if 'atr' in state and state['atr'] is not None:
            self.atr = float(state['atr'])
        
        if '_atr_filtered_signals' in state:
            self._atr_filtered_signals = int(state['_atr_filtered_signals'])
        ...
        if '_trailing_stop_triggered' in state:
            self._trailing_stop_triggered = int(state['_trailing_stop_triggered'])
        
        if '_position' in state:
            self._position = int(state['_position'])
        
        if '_position_size' in state:
            self._position_size = int(state['_position_size'])
        
        if '_entry_price' in state and state['_entry_price'] is not None:
            self._entry_price = float(state['_entry_price'])
        
        if '_trailing_stop_active' in state:
            self._trailing_stop_active = bool(state['_trailing_stop_active'])
        
        if '_trailing_stop_price' in state and state['_trailing_stop_price'] is not None:
            self._trailing_stop_price = float(state['_trailing_stop_price'])
        
        if '_highest_price_since_entry' in state and state['_highest_price_since_entry'] is not None:
            self._highest_price_since_entry = float(state['_highest_price_since_entry'])
        
        if '_lowest_price_since_entry' in state and state['_lowest_price_since_entry'] is not None:
            self._lowest_price_since_entry = float(state['_lowest_price_since_entry'])
        
        if '_total_capital' in state and state['_total_capital'] is not None:
            self._total_capital = float(state['_total_capital'])
        ...
        atr_status = f", ATR={self.atr:.2f}" if self.atr is not None else ""
        self.logger.info(
            f"策略状态已恢复: 短期均线={self.short_ma}, 长期均线={self.long_ma}"
            f"{rsi_status}{atr_status}, 信号={self.signal.value}, 持仓={self._position}"
        )